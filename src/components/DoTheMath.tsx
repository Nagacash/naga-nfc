...
const DoTheMath: React.FC = () => {
  ...
  return (
    <section id="do-the-math" className="py-20 bg-brand-primary-base text-foreground px-6">
      <div className="container mx-auto text-center">
        <h2 className="text-3xl md:text-4xl font-display text-foreground mb-4">Do the Math: Your Profit Margins</h2>
        

        {Object.entries(cardCosts).map(([quantity, costPerCard]) => (
          <div key={quantity} className="mb-12">
            <h3 className="text-2xl font-bold text-brand-accent mb-6">{quantity} pcs Bundle</h3>
            <p className="text-foreground mb-6">Your cost per card: €{costPerCard.toFixed(2)}</p>
            <div className="overflow-x-auto">
              <table className="min-w-full bg-brand-secondary/10 rounded-lg overflow-hidden shadow-lg mb-8 mx-auto">
                <thead>
                  <tr className="bg-brand-accent text-foreground">
                ...
                <tbody>
                  {sellingPrices.map((price) => {
                    ...
                    return (
                      <tr key={price} className="border-b border-brand-primary-base last:border-b-0">
        ...
        <p className="text-foreground max-w-2xl mx-auto mt-12">
