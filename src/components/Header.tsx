...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || isMenuOpen ? 'bg-white shadow-lg' : 'bg-white'}`}>
        <div className="container mx-auto px-6 py-6 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-8">
            ...
            <a href="#how" className="text-gray-800 hover:text-black transition-colors duration-200">How It Works</a>
            <a href="#do-the-math" className="text-gray-800 hover:text-black transition-colors duration-200">Do the Math</a>
            <a href="#contact" className="text-gray-800 hover:text-black transition-colors duration-200">Contact</a>
          </nav>
          ...
          <div className="md:hidden">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="z-50" aria-label="Toggle menu">
              {isMenuOpen ? <CloseIcon className="w-8 h-8 text-gray-800" /> : <MenuIcon className="w-8 h-8 text-gray-800" />}
            </button>
          </div>
        </div>
      </header>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`md:hidden fixed inset-0 z-40 bg-white flex flex-col items-center justify-center space-y-8 transition-opacity duration-300 ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <a href="#offer" onClick={() => setIsMenuOpen(false)} className="text-2xl text-gray-800 hover:text-black transition-colors">What We Offer</a>
